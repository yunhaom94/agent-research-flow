...
def main():
    ...
    parser.add_argument('--format', choices=['json', 'compact'], default='compact',
                        help='Output format: compact (scannable digest, default) or json (full records)')
    ...
    if args.format == 'json':
        ...
        write_compact(unique_papers, sys.stdout)


def write_compact(papers: list, out) -> None:
    """Emit a scannable digest: one block per paper with title, IDs, and full abstract."""
    for idx, p in enumerate(papers):
        ...
        pdf_marker = 'pdf=yes' if p.get('pdf_url') else 'pdf=no'
        header_bits = [
            f"[{idx}]",
            str(p.get('year') or '----'),
            p.get('source', ''),
        ...
        out.write(' | '.join(b for b in header_bits if b) + '\n')
        out.write(f"  Title: {p.get('title', '').strip()}\n")
